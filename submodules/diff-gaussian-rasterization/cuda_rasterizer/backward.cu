...
__device__ void computeCov3D(int idx, float gaussian_type, const glm::vec3 scale, float mod, const glm::vec4 rot, const float* dL_dcov3Ds, glm::vec3* dL_dscales, glm::vec4* dL_drots)
{
...
// 	dL_dmean_d.x = (view[2] - view[3] * mul3) * dL_ddepth[idx];
// 	dL_dmean_d.y = (view[6] - view[7] * mul3) * dL_ddepth[idx];
// 	dL_dmean_d.z = (view[10] - view[11] * mul3) * dL_ddepth[idx];
	dL_dmean_d.x = (view[2]) * dL_ddepth[idx];
	dL_dmean_d.y = (view[6]) * dL_ddepth[idx];
	dL_dmean_d.z = (view[10]) * dL_ddepth[idx];
	...
	if (scales)
		computeCov3D(idx, gaussian_type[idx], scales[idx], scale_modifier, rotations[idx], dL_dcov3D, dL_dscale, dL_drot);
}
...
// 	    }
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float dpixel_ddepth = alpha * T;
...
// 			}
			...
			atomicAdd(&(dL_ddepths[global_id]), dpixel_ddepth * dL_dpixel_d);
